...
interface BarcodeDao {
    @Query("SELECT * FROM barcode_cache WHERE barcode = :code LIMIT 1")
    suspend fun getByBarcode(code: String): BarcodeEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(entity: BarcodeEntity)

    @Query("SELECT COUNT(*) > 0 FROM barcode_cache WHERE barcode = :code")
