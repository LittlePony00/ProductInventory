...
`domain/port/inbound` задает use-case контракты: `IAuthService`, `IHouseholdService`, `IProductService`, `ICategoryService`, `IBarcodeProductService`, `IProductEnrichmentService`, `IRecommendationService`, `INotificationService`, `INotificationPreferencesService`, `IUserService`.
...
- Локальные напоминания о сроке годности и низком остатке планируются на мобильных устройствах по кешу продуктов, чтобы работать без интернета.
...
```mermaid
flowchart TB
    Products[Cached products on device] --> Planner[ProductLocalReminderPlanner]
    Settings[NotificationSettings] --> Planner
    Planner --> Android[Android AlarmManager]
    Planner --> IOS[iOS UNUserNotificationCenter]
    Android --> LocalAndroid[System local notification]
    IOS --> LocalIOS[System local notification]
```

`ProductLocalReminderPlanner` создает два типа локальных reminder:

- `EXPIRATION` по `expirationDate - expirationReminderDays`;
- `LOW_STOCK` сразу, когда `remainingAmount <= lowStockThreshold`.

На устройстве применяются настройки:

- `expirationRemindersEnabled`;
- `lowStockRemindersEnabled`;
- `expirationReminderDays`.

Если settings недоступны офлайн, используется дефолт shared `NotificationSettings`.
